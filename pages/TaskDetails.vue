...
const hintSteps = [
    ...
    {selector: '.tas__name-wrapper', text: 'stepHint.goal'},
    ...
    {selector: '.phrase', text: 'stepHint.remaining'},
   ]
...
function handleHintFinish() {
    const user = getAuth().currentUser
    if (user?.uid) {
        localStorage.setItem(`hints_details_${user.uid}`, 'true')
    }
...
onMounted(() => {
    const user = getAuth().currentUser
    if (user?.uid && localStorage.getItem(`hints_details_${user.uid}`) !== 'true') {
