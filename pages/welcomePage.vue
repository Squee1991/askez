                                                    ...
                                                    <div class="task__datum-wrapper">
                                                        <div class="task__goal-wrapper">
                                                            <div :style="{ color: task.color }"
                                                                 class="task__goal-item goals goal__name">{{
                                                                task.goal
                                                                }}
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="task__progress-wrapper">
                                                        <div class="task__progress-value">
                                                            <div class="task__progress-green">
                                                                ...
                                                                <span class="progress-green percent__progress">{{
                                                                    getProgress(task).progress
                                                                    }}</span>
                                                            </div>
                                                            <div class="task__progress-green">
                                                                <span class="emoji">&#10060;</span>
                                                                <span class="progress-red percent__progress">{{
                                                                    getProgress(task).progressMiss
                                                                    }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
...
const stepHintRef = ref(null)
const hasTasks = ref(false)
const hintSteps = [
    {selector: '.goal__selector', text: 'stepHint.selector'},
    {selector: '.calendar-wrapper', text: 'stepHint.calendar'},
    {selector: '.task__goal-list', text: 'stepHint.taskList'},
    {selector: '.task__come-btn', text: 'stepHint.detailsButton'},
    {selector: '.goal__btn', text: 'stepHint.premium'},
    {selector:'plus__icon-border', text: 'create goal'},
...
const fallbackSteps = [
    ...
    {selector: '.calendar-wrapper', text: 'stepHint.calendar'},
    {selector: '.goal__btn', text: 'stepHint.premium'},
    {selector:'.footer__nav-btn__plus', text: 'stepHint.plus'},
    {selector: '.goal__selector', text: 'stepHint.selector'},
    {selector: '.calendar-wrapper', text: 'stepHint.calendar'},
    {selector: '.task__goal-list', text: 'stepHint.taskList'},
    {selector: '.task__come-btn', text: 'stepHint.detailsButton'},
...
const activeHintSteps = computed(() => hasTasks.value ? allSteps : fallbackSteps)
const route = useRoute();
const isTasksLoaded = computed(() => tasks.value.length > 0)
...
function handleFinish() {
    ...
    if (user && user.uid) {
        const key = `hints_welcome_${user.uid}`
...
onMounted(() => {
    ...
    if (user && user.uid) {
        ...
        nextTick(() => {
            const elements = document.querySelectorAll('.task__goal-list')
            hasTasks.value = elements.length > 0
        })
    }
})
watch(showHints, (val) => {
    if (val) {
        nextTick(() => {
            stepHintRef.value?.retryHighlight()
        })
    }
})
