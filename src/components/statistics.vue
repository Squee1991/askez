<script setup>
import {useHabitStore} from '../../stores/habitStore.js'
import {useJourneyStore} from '../../stores/useJourneyStore.js'
import {computed, ref} from 'vue'
import CheckedIcon from '../../assets/images/checkIcon.svg'
import CheckedNotFull from '../../assets/images/cross.svg'
import InProgressIcon from '../../assets/images/in-progress.svg'

const isActiveButton = ref('active')

const habitStore = useHabitStore()
const journeyStore = useJourneyStore()
const {goals} = storeToRefs(journeyStore)

const tasks = computed(() => {
    const archiveIds = new Set(habitStore.archiveTasks.map(task => task.id))
    return habitStore.tasks.filter(task =>
        isActiveButton.value === 'active' ? !archiveIds.has(task.id) : false
    )
})

const filteredTotal = computed(() => tasks.value.length)

const filteredDone = computed(() =>
    tasks.value.filter(t => t.progress === 100 && t.progressMiss === 0).length
)

const filteredNotCompleted = computed(() =>
    tasks.value.filter(t => t.progress + t.progressMiss === 100 && t.progress < 100).length
)

const filteredInProgress = computed(() =>
    tasks.value.filter(t => t.progress + t.progressMiss < 100).length
)

const filteredCompletionRate = computed(() => {
    const relevantTasks = tasks.value
    if (!relevantTasks.length) return 0
    const done = relevantTasks.filter(t => t.progress === 100 && t.progressMiss === 0).length
    return Math.round((done / relevantTasks.length) * 100)
})

const currenticon = (task) => {
    if (task.progress === 100) return CheckedIcon
    if (task.progress + task.progressMiss === 100) return CheckedNotFull
    return InProgressIcon
}

const completionColor = computed(() => {
    const progress = filteredCompletionRate.value
    if (progress === 100) return "#388E3C"
    if (progress >= 80) return "#38b840"
    if (progress >= 60) return "#FFA500"
    if (progress >= 40) return "#D38B5D"
    if (progress >= 20) return "#b24c39"
    if (progress > 0) return "#a10505"
    return "grey"
})

const availableTotal = computed(() => goals.value.length)
const availableStarted = computed(() =>
    goals.value.filter(g => g.progress > 0 && g.progress < g.target).length
)
const availableCompleted = computed(() =>
    goals.value.filter(g => g.progress >= g.target).length
)
const availableCompletion = computed(() => {
    const total = goals.value.length
    if (!total) return 0
    const completed = goals.value.filter(g => g.progress >= g.target).length
    return Math.round((completed / total) * 100)
})
</script>


<template>
    <div class="stats__content">
        <div class="stats__header">
            <div class="stats__banner-content">
                <div class="stats__left">
                    <div class="stats__text">{{ $t('navTop.stats') }}</div>
                    <template v-if="isActiveButton === 'active'">
                        <div v-if="tasks.length">
                            <div class="total__task">
                                {{ $t('statslable.total') }}: <span class="askez__counter">{{ filteredTotal }}</span>
                            </div>
                            <div class="total__task">
                                {{ $t('statslable.doneTotal') }}: <span class="askez__counter">{{ filteredDone }}</span>
                            </div>
<!--                            <div class="total__task">-->
<!--                                {{ $t('statslable.notCompleted') }}: <span-->
<!--                                    class="askez__counter">{{ filteredNotCompleted }}</span>-->
<!--                            </div>-->
                            <div class="total__task">
                                {{ $t('statslable.InProgressTotal') }}: <span
                                    class="askez__counter">{{ filteredInProgress }}</span>
                            </div>
                        </div>
                        <div class="not__active-askez" v-else>
                            {{ $t('hasNotAskez.value') }}
                        </div>
                    </template>
                    <template v-else>
                        <div v-if="availableTotal">
                            <div class="total__task">
                                {{ $t('statslable.total') }}: <span class="askez__counter">{{
                                availableTotal
                                }}</span>
                            </div>
                            <div class="total__task">
                                {{ $t('statslable.doneTotal') }}: <span class="askez__counter">{{ availableCompleted }}</span>
                            </div>
                            <div class="total__task">
                                {{ $t('statslable.InProgressTotal') }}: <span class="askez__counter">{{
                                availableStarted
                                }}</span>
                            </div>


                        </div>
                        <div class="not__active-askez" v-else>
                            {{ $t('journey.empty') }}
                        </div>
                    </template>
                </div>
                <div class="stats__right" :style="{ color: completionColor }">
                    <template v-if="isActiveButton === 'active'">
                        {{ filteredCompletionRate }}<span class="procent">%</span>
                    </template>
                    <template v-else>
                        {{ availableCompletion }}<span class="procent">%</span>
                    </template>
                </div>
            </div>
            <div class="select__inner">
                <div class="select__wrapper">
                    <div class="indicator"
                         :style="{ transform: isActiveButton === 'active' ? 'translateX(0%)' : 'translateX(100%)' }"/>
                    <span class="select"
                          :class="{ 'select__active': isActiveButton === 'active' }"
                          @click="isActiveButton = 'active'">{{ $t('statslable.active') }}</span>
                    <span class="select"
                          :class="{ 'select__active': isActiveButton === 'archive' }"
                          @click="isActiveButton = 'archive'">{{ $t('statslable.journey') }}</span>
                </div>
            </div>
        </div>
        <div class="stat__askez-scroll">
            <div class="stat__askez-wrapper">
                <template v-if="isActiveButton === 'active'">
                    <div v-for="task in tasks" :key="task.id" class="stat__askez active-task">
                        <div class="stat__askez__inner" >
                            <div class="askez__info">
                                <div class="askez__name">{{ task.goal }}</div>
                            </div>
                            <div class="askez__titles">
                                <!--                           <div class="askez__title-status &#45;&#45;name__askez">{{ $t('infoLabels.name') }}</div>-->
                                <div class="askez__title-status --status__askez">{{ $t('infoLabels.status') }}</div>
                                <img class="askez__status-img" :src="currenticon(task)" alt=""/>
                            </div>
                        </div>

                    </div>
                </template>
                <template v-else>
                    <div v-for="goal in goals" :key="goal.id" class="stat__askez archived-task">
                        <div class="journey__stat">
                            <div class="askez__titles">
                                <div class="askez__name">{{ $t(`Task.${goal.i18nKey}`) }}</div>
                                <!--                            <div class="askez__title-status &#45;&#45;name__askez">{{ $t('infoLabels.name') }}</div>-->
                            </div>
                            <div class="askez__info">
                                <div class="way__progress">{{ goal.progress }} / {{ goal.target }} {{ goal.unit }}</div>
                            </div>
                        </div>
                    </div>
                </template>
            </div>
        </div>
    </div>
</template>

<style scoped>

.way__progress {
    color: var(--text-color);
    font-family: "Nunito", sans-serif;
    font-size: 19px;
    font-weight: 600;
}

.stat__askez__inner {
    display: flex;
    justify-content: space-between;
    align-items: center;
    padding: 5px 0;
}

.journey__stat{
 display: flex;
    justify-content: space-between;
    padding: 14px 5px;
}

.stats__left {
    min-height: 140px;
}

.active-task {
    background: var(--menu--btn-bg);
}

.archived-task {
    background: var(--menu--btn-bg);
    opacity: 0.8;
}

.select__active {
    color: white !important;
}

.select__wrapper {
    position: relative;
    display: flex;
    justify-content: space-between;
    border-radius: 10px;
    overflow: hidden;
}

.indicator {
    position: absolute;
    top: 0;
    left: 0;
    width: 50%;
    height: 100%;
    background: var(--indicator-bg);
    border-radius: 30px;
    z-index: 0;
    pointer-events: none;
    transition: transform 0.35s cubic-bezier(0.22, 1, 0.36, 1);
    will-change: transform;
}

.select {
    z-index: 1;
    position: relative;
    width: 50%;
    color: gainsboro;;
    font-family: "Nunito", serif;
    letter-spacing: 1px;
    padding: 10px 0;
    text-align: center;
    cursor: pointer;
    transition: color 0.3s ease;
}

.select__inner {
    margin: 10px 0;
    padding: 4px;
    background-color: var(--slider-bg);
    border-radius: 30px;
}

.not__active-askez {
    color: var(--text-color);
    font-size: 16px;
    font-family: "Nunito", sans-serif;
    font-weight: 400;
    margin-top: 10px;
}


.stat__askez-wrapper {
    flex-grow: 1;
    overflow-y: auto;
    padding-bottom: 10px;
}

.stat__askez-scroll {
    flex-grow: 1;
    overflow-y: auto;
}

.askez__status-img {
    width: 30px;
    margin-right: 10px;
    display: flex;
    justify-content: center;
    align-items: center;
}

.stats__content {
    display: flex;
    flex-direction: column;
    height: 100%;
}

.stats__header {
    flex-shrink: 0;
}

.stats__text {
    letter-spacing: 1px;
    font-size: 28px;
    color: var(--text-color);
    font-family: "Nunito", serif;
    font-weight: 600;
}

.stats__banner-content {
    padding: 8px 15px 8px 20px;
    background: var(--menu--btn-bg);
    border-radius: 15px;
    display: flex;
    justify-content: space-between;
    align-items: center;
    min-height: 155px;
    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05);
    margin-bottom: 15px;
}

.stats__right {
    display: flex;
    justify-content: center;
    align-items: center;
    border-radius: 50%;
    font-size: 28px;
    font-weight: bold;
    font-family: "Acme", serif;
    background: #D0EBFF;
    width: 100px;
    height: 100px;
    padding: 15px;
}

.procent {
    font-size: 22px;
    padding: 1px;
}

.askez__name {
    display: flex;
    align-items: center;
    letter-spacing: 1px;
    font-size: 20px;
    font-weight: 400;
    font-family: "Nunito", sans-serif;
    color: var(--text-color);
}

.total__task {
    color: var(--text-color);
    display: flex;
    align-items: center;
    font-family: "Nunito", serif;
    letter-spacing: 1px;
}

.askez__counter {
    font-size: 18px;
    font-weight: bold;
    margin-left: 5px;
}

.askez__titles {
    padding: 8px 10px;
    display: flex;
    font-size: 18px;
    justify-content:center;
    align-items: end;
    flex-direction: column;
}

.askez__title-status {
    letter-spacing: 1px;
    font-size: 20px;
    font-weight: bold;
    color: var(--text-color);
    font-family: "Nunito", serif;
}

.stat__askez {
    border-radius: 15px;
    padding: 0 15px 0 10px;
    margin-bottom: 10px;
    /*min-height: 81px;*/
}

.askez__info {
    padding: 10px;
    display: flex;
    justify-content: space-between;
}
</style>